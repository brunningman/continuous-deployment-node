...
export default function readyPromiseAssertions(fetchMock, assert) {
  ...
  assert.test(function (t) {
    ...
    fetchMock.get(config.urls.sdk + '/mySegments/facundo@split.io', mySegmentsFacundo, {
      delay: fromSecondsToMillis(config.startup.requestTimeoutBeforeReady) - 20
    });
  ...
  assert.test(function (t) {
    ...
    fetchMock.get(config.urls.sdk + '/mySegments/nicolas@split.io', mySegmentsFacundo); // shared client endpoint configured to emit SDK_READY_TIMED_OUT
